...
BoxOffice is a layered backend. Each layer has one job and a clean seam to the next,
...
```
 domain ──▶ data ──▶ fraud ──▶ agents ──▶ eval
   │          │         │         │         │
 market-   seeded    graph +    backend-   leaderboard
 place     tenant    ML +      agnostic    vs ground
 model     + GT      causal    workflows   truth
                                  │
                                 llm (analytical | simulated | Claude)
```

## 1 · Domain (`boxoffice/domain`)

Pydantic models for the SeatGeek money graph — `Company`, `Employee`, `SeatGeekCard`
(physical/virtual, vendor-lockable), `SeatGeekCashAccount` (operating/float/reserve),
`Merchant`, `CardTransaction`, `CashTransaction`, `Dispute` (real Visa/MC chargeback
reason codes), and `SpendPolicy`. Every transaction carries a `GroundTruth` block
...
## 2 · Data (`boxoffice/data`)

A seeded generator (`numpy.default_rng`) materializes one SeatGeek tenant: ~40 employees
across the marketplace org, ~70 vendors/venues, ~5,900 card transactions over 90 days,
cash money-movement (nightly card sweeps, ticketing-settlement ACH revenue, vendor bill
pay, **settlement-float yield accrual**), and chargeback disputes.

The realism is the point — it's engineered so **no single feature separates fraud**:

- **Legit partner travel** produces geo jumps on *trusted* devices (Partnerships/Exec).
...
- **Fraud has a gradient**: blatant foreign gift-card bursts → stealth domestic mule
  rings → **account-takeover that rides the buyer's own device** (the irreducible tail
  that honestly caps recall).
- Merchant risk **overlaps** both classes.

Same seed → byte-identical tenant → reproducible leaderboard.

## 3 · Fraud (`boxoffice/fraud`)
...
- **`graph.py`** — heterogeneous card/device/IP/merchant graph (networkx). Cards are
  ...
  random across the base; office IPs are single-metro and excluded — a
  **count-independent** heuristic). Connected components ≥ 2 cards are candidate
  bot/scalper rings, scored by shared-device presence, merchant risk, foreign and
  odd-hour share. Ring exposure is scoped to the *suspicious* shared-infrastructure
  transactions.
...
## 5 · Agents (`boxoffice/agents`)

Five single-shot evaluable agents (categorization, policy audit, chargeback dispute,
fraud triage, tieout) plus two multi-step workflows. Shared `inputs.py` builders turn a
transaction into agent payloads (reading only observable facts) so the orchestrator and
the eval harness never drift. The **orchestrator** sequences the specialists and
escalates to the **investigator** on high risk; both emit full traces.

## 6 · Eval (`boxoffice/eval`)
