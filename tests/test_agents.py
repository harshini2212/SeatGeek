...
from boxoffice.agents import (
    CategorizationAgent,
    BoxOfficeOrchestrator,
    FraudInvestigator,
    PolicyAuditAgent,
)
from boxoffice.agents import inputs as build
from boxoffice.agents.categorization import recoverable_from_name
from boxoffice.llm import AnalyticalBackend
...
def test_orchestrator_produces_decision(dataset, pipeline):
    ...
    dec = BoxOfficeOrchestrator(dataset, pipeline, AnalyticalBackend()).handle_transaction(
        top.txn_id)
