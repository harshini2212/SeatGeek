...
from boxoffice.api import app
from boxoffice.exports import delivery as D
from boxoffice.exports import serializers as S
from boxoffice.exports.scheduler import ScheduleStore, next_run_after
...
def _sched(**kw):
    base = dict(id="exp_x", name="Test export", dataset="cards", recipient="f@seatgeek.com",
...
def test_run_one_writes_artifact_and_advances(tmp_path):
    ...
    s = store.create(name="Daily cards", dataset="cards", cadence="daily", recipient="f@seatgeek.com")
    run = store.run_one(s)
...
def test_run_due_only_fires_past_due_enabled(tmp_path):
    ...
    due = store.create(name="hourly", dataset="people", cadence="hourly", recipient="f@seatgeek.com")
    store.update(due.id, enabled=True)
    paused = store.create(name="paused", dataset="cards", cadence="hourly", recipient="f@seatgeek.com")
...
def test_invalid_dataset_or_cadence_rejected(tmp_path):
    ...
    for kwargs in ({"dataset": "nope", "cadence": "daily"}, {"dataset": "cards", "cadence": "yearly"}):
        try:
            store.create(name="x", recipient="f@seatgeek.com", **kwargs)
...
def test_delete_removes_run_artifacts(tmp_path):
    ...
    store = _store(tmp_path, lambda: t)
    s = store.create(name="temp", dataset="cards", cadence="daily", recipient="f@seatgeek.com")
    run = store.run_one(s)
...
def test_store_persists_across_reload(tmp_path):
    ...
    sched = s1.create(name="keep", dataset="invoices", cadence="weekly", recipient="f@seatgeek.com")
...
def test_schedule_api_create_run_download_delete():
    created = client.post("/api/exports/schedules", json={
        ...
        "recipient": "controller@seatgeek.com", "filters": {"flagged": True}}).json()
    sid = created["id"]
...
def test_schedule_api_rejects_bad_dataset():
    r = client.post("/api/exports/schedules", json={
        "name": "bad", "dataset": "ledger", "cadence": "daily", "recipient": "x@seatgeek.com"})
    assert r.status_code == 400


# ---- email delivery ------------------------------------------------------- #
def test_build_email_is_multipart_with_csv_attachment():
    msg = D.build_email("boxoffice@seatgeek.com", _sched(name="Weekly cards"),
                        b"card_id,spend\r\nC1,100\r\n", "cards.csv", NOW, 1)
    assert msg["To"] == "f@seatgeek.com" and "Weekly cards" in msg["Subject"]
...
def test_smtp_delivery_calls_smtplib(monkeypatch):
    ...
    res = D.Delivery(cfg).deliver(_sched(recipient="cfo@seatgeek.com"), b"x\r\n1\r\n", "c.csv", NOW, 1)
    assert res.channel == "smtp" and res.ok
    assert ("starttls",) in sent and ("login", "u") in sent
    assert any(s[0] == "send" and s[1] == "cfo@seatgeek.com" for s in sent)
...
def test_run_one_attaches_delivery_record(tmp_path):
    ...
    s = store.create(name="Daily people", dataset="people", cadence="daily", recipient="f@seatgeek.com")
    run = store.run_one(s)
...
def test_api_email_preview_and_eml_download():
    created = client.post("/api/exports/schedules", json={
        ...
        "recipient": "controller@seatgeek.com"}).json()
    sid = created["id"]
    run = client.post(f"/api/exports/schedules/{sid}/run").json()
    assert run["eml"] is True and run["delivery"] is not None
    preview = client.get(f"/api/exports/schedules/{sid}/runs/{run['id']}/email").json()
    assert preview["to"] == "controller@seatgeek.com" and "Email preview test" in preview["subject"]
