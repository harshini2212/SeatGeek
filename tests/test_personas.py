...
from boxoffice.agents.agent_tools import ToolContext, allowed_tools, replay_policy, tool_names
from boxoffice.agents.personas import PERSONA_TABS, Persona
...
def test_ap_agent_refuses_to_pay_duplicate(dataset, pipeline):
    from boxoffice.agents.agent_tools import detect_duplicate_invoices, pay_invoice
    ctx = ToolContext(dataset, pipeline)
...
def test_ap_catches_bank_change_and_over_po(dataset, pipeline):
    from boxoffice.agents.agent_tools import three_way_match, vendor_bank_change_check
    ctx = ToolContext(dataset, pipeline)
