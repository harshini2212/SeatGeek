"""BoxOffice command-line interface.

Subcommands:
  generate     build a synthetic SeatGeek tenant and print its summary
  ...
  orchestrate  run the full BoxOffice agent on a transaction (any backend)
...
def cmd_orchestrate(args, cfg) -> None:
    ...
    txn_id = _pick_txn(pipe, args.txn, args.top)
    decision = BoxOfficeOrchestrator(ds, pipe, backend).handle_transaction(txn_id)
    if args.json:
        print(json.dumps(decision.to_dict(), indent=2))
        return
    R.rule("BoxOffice orchestrator")
...
def cmd_serve(args, cfg) -> None:
    import uvicorn
    R.info(f"serving BoxOffice API on http://{args.host}:{args.port} (docs at /docs)")
    uvicorn.run("boxoffice.api.app:app", host=args.host, port=args.port, reload=False)


# --------------------------------------------------------------------------- #
def build_parser(cfg) -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="boxoffice", description=__doc__,
                                formatter_class=argparse.RawDescriptionHelpFormatter)
    sub = p.add_subparsers(dest="cmd", required=True)

    g = sub.add_parser("generate", help="build a synthetic SeatGeek tenant")
    ...
    o = sub.add_parser("orchestrate", help="run the full BoxOffice agent")
