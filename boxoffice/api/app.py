...
like an internal SeatGeek service: fraud scoring, autonomous investigation, the
...
app = FastAPI(
    title="BoxOffice",
    description="Agentic AI + financial-correctness evaluation for SeatGeek spend "
                "(SeatGeek Card & SeatGeek Treasury).",
    version="0.1.0",
...
_EXPORTS_DIR = (Path(tempfile.gettempdir()) / "boxoffice_exports"
...
def api_info() -> dict[str, Any]:
    ...
    return {
        "service": "boxoffice",
        "tagline": "AI marketplace finance & trust OS for SeatGeek — a permission-scoped agent for every role",
...
def dashboard() -> str:
    """SeatGeek-themed single-page dashboard over the API."""
...
def orchestrate(req: OrchestrateRequest) -> dict[str, Any]:
    from ..agents import BoxOfficeOrchestrator
    ...
    return BoxOfficeOrchestrator(ds, pipe, backend).handle_transaction(txn_id).to_dict()
...
def _build_insights_impl(seed: int) -> list[dict[str, Any]]:
    ...
    if yield_usd > 0:
        add("medium", "Treasury", "trending", "Idle-cash yield available",
            "Sweep the operating buffer into SeatGeek Treasury MMF (~4.1% APY).",
...
def api_research_analyze(req: AnalyzeReq) -> dict[str, Any]:
    ...
    out = cc.complete_json(
        system=("You are a corporate-strategy analyst at SeatGeek reviewing a public "
                "live-entertainment peer's SEC-filed XBRL figures. Ground every claim in "
                "the numbers given; never invent figures. Be direct and quantitative."),
        user=("Analyze this company as (a) a competitor or venue/rights partner in live "
              "entertainment and (b) a benchmark for our own spend discipline. Treat "
              "'underwriting_view' as how you'd view them as a counterparty/partner. All "
              "accounting tie-outs shown were recomputed from the filing.\n\n"
...
def api_research_improve(req: AnalyzeReq) -> dict[str, Any]:
    """SeatGeek AI's spend-improvement plan for a company — grounded in its filed cost lines."""
    from ..ai import ClaudeClient
    from ..research.edgar import cached_profile
    cc = ClaudeClient(model=req.model)
    if not cc.available:
    ...
    out = cc.complete_json(
        system=("You are SeatGeek AI, a spend-management strategist. Given a company's as-filed "
                "cost structure and a computed addressable-spend base, propose concrete, "
                "technical ways SeatGeek would cut or optimize their spend. Ground every dollar "
...
def api_research_agent(req: AgentReq) -> dict[str, Any]:
    ...
    profile = cached_profile(req.ticker.upper())
    run = cc.run_agent(
        system=("You are SeatGeek's autonomous spend-optimization agent working on "
...
def api_ai_brief(req: BriefReq) -> dict[str, Any]:
    ...
    out = cc.complete_json(
        system=("You are SeatGeek AI writing a CFO's morning brief from live spend data. "
...
def api_cards_agent(req: CardAgentReq) -> dict[str, Any]:
    """Deploy an agentic Claude workflow that audits the SeatGeek card program on live data —
    ...
    run = cc.run_agent(
        system=("You are SeatGeek's autonomous card-program agent for this company. Work the stated "
...
class ScheduleCreate(BaseModel):
    ...
    dataset: str
    cadence: str
    recipient: str = "finance@seatgeek.com"
