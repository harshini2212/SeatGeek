...
class SMTPConfig:
    ...
    sender: str = "boxoffice@seatgeek.com"
    ...
    def from_env(cls) -> "SMTPConfig":
        ...
        return cls(
            ...
            sender=os.environ.get("SMTP_FROM", "boxoffice@seatgeek.com"),
...
def build_email(sender: str, sched: Any, csv_bytes: bytes, filename: str,
    ...
    msg["Subject"] = f"[BoxOffice] {sched.name} — {now:%b %d, %Y}"
    msg["Date"] = format_datetime(now)
    msg["Message-ID"] = make_msgid(domain="boxoffice.seatgeek.com")
    msg["X-BoxOffice-Schedule"] = sched.id
    msg["X-BoxOffice-Dataset"] = sched.dataset
    msg.set_content(
        ...
        f"— BoxOffice, automated finance exports for SeatGeek\n")
