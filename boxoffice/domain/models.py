"""Pydantic domain models for the SeatGeek spend graph.

These are the canonical records the platform reasons over: companies, employees,
SeatGeek Cards, SeatGeek Treasury accounts, merchants, card and cash transactions, disputes,
...
class SeatGeekCard(BaseModel):
    ...
    # SeatGeek virtual cards are frequently vendor-locked.
    locked_merchant_id: Optional[str] = None


class SeatGeekCashAccount(BaseModel):
    id: str
    company_id: str
    balance_cents: int
    apy: float
...
class Dataset(BaseModel):
    """A fully-materialized synthetic SeatGeek tenant."""

    company: Company
    policy: SpendPolicy
    employees: list[Employee]
    cards: list[SeatGeekCard]
    cash_accounts: list[SeatGeekCashAccount]
    ...
    def card_index(self) -> dict[str, SeatGeekCard]:
