"""Dynamic partner/seller-trust underwriting — SeatGeek's payout-risk model, probabilistic.

A live-events marketplace extends trust to sellers and venue partners: it pays them out
for tickets before the event clears, so it is effectively underwriting each counterparty's
likelihood of causing a loss (a chargeback, a non-delivery, a bust-out). We make that
quantitative: a gradient-boosted probability-of-loss model trained on a synthetic partner
portfolio, plus a cash-coverage limit recommendation and a dynamic action
(raise / hold / reduce-within-24h) — trust scales up as clean history builds and can be
cut fast for at-risk accounts.
"""
...
class Underwriter:
    ...
    def assess(self) -> CreditAssessment:
        ...
        # ones are capped well below it. (SeatGeek daily-pay limit ~ aggregate balance.)
