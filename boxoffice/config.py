...
def load_config() -> Config:
    ...
    default_backend = os.getenv("BOXOFFICE_DEFAULT_BACKEND", "claude-opus-4-8").strip()
    cfg = Config(
        seed=int(os.getenv("BOXOFFICE_SEED", "7")),
        anthropic_api_key=key,
        default_backend=default_backend or "claude-opus-4-8",
        effort=os.getenv("BOXOFFICE_EFFORT", "high").strip() or "high",
    )
