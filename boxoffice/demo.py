...
Run via ``boxoffice demo`` or ``python -m boxoffice.demo``. Generates a SeatGeek
...
def run_demo(seed: int = 7, eval_limit: int = 30) -> None:
    from .agents import BoxOfficeOrchestrator, FraudInvestigator
    ...
    # 1) Tenant -----------------------------------------------------------------
    R.rule("1 / Synthetic SeatGeek tenant (SeatGeek Card + SeatGeek Treasury)")
    ...
    # 4) Orchestrator -----------------------------------------------------------
    R.rule("4 / BoxOffice orchestrator — one agent, many tasks, one decision")
    decision = BoxOfficeOrchestrator(ds, pipe, AnalyticalBackend()).handle_transaction(target)
    R.orchestrator_decision(decision.to_dict())
    # also resolve a real dispute end-to-end
    if ds.disputes:
        dtxn = ds.disputes[0].transaction_id
        ddec = BoxOfficeOrchestrator(ds, pipe, AnalyticalBackend()).handle_transaction(dtxn)
        if ddec.dispute:
