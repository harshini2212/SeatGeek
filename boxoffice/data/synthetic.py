"""Deterministic synthetic SeatGeek tenant generator.

Produces a fully-materialized :class:`~boxoffice.domain.models.Dataset` — a
representative slice of SeatGeek's own corporate money graph: employees across the
marketplace org, their SeatGeek Cards, SeatGeek Treasury accounts, the vendors and
venues SeatGeek pays, ~thousands of card transactions, cash money-movement, and
chargeback disputes.
...
# SeatGeek's org — the teams that carry a corporate card at a live-events marketplace.
DEPARTMENTS = ("Engineering", "Marketplace", "Partnerships", "Marketing",
               "Trust & Safety", "Finance", "Live Operations", "Executive")
...
DEPT_CATEGORIES: dict[str, list[ExpenseCategory]] = {
    ...
    "Marketplace": [ExpenseCategory.SOFTWARE, ExpenseCategory.PROFESSIONAL_SERVICES, ExpenseCategory.MEALS],
    "Partnerships": [ExpenseCategory.TRAVEL, ExpenseCategory.MEALS, ExpenseCategory.SOFTWARE],
    "Marketing": [ExpenseCategory.ADVERTISING, ExpenseCategory.SOFTWARE, ExpenseCategory.MEALS],
    "Trust & Safety": [ExpenseCategory.SOFTWARE, ExpenseCategory.PROFESSIONAL_SERVICES, ExpenseCategory.MEALS],
    "Finance": [ExpenseCategory.PROFESSIONAL_SERVICES, ExpenseCategory.SOFTWARE, ExpenseCategory.OFFICE],
    "Live Operations": [ExpenseCategory.OFFICE, ExpenseCategory.SHIPPING, ExpenseCategory.UTILITIES, ExpenseCategory.RENT],
...
# Vendor name fragments per category — the real infra, venue and marketing suppliers a
# live-events marketplace like SeatGeek actually pays, so generated merchants read true.
MERCHANT_NAMES: dict[ExpenseCategory, list[str]] = {
    ExpenseCategory.SOFTWARE: ["Datadog", "Snowflake", "AWS", "Twilio", "Segment", "GitHub", "Braze", "PagerDuty"],
    ExpenseCategory.HARDWARE: ["Apple Store", "Zebra Scanners", "CDW", "Boca Ticket Printers", "Elo POS"],
    ExpenseCategory.TRAVEL: ["United Airlines", "Delta", "Marriott", "Hilton", "Uber", "Lyft", "Airbnb"],
    ExpenseCategory.MEALS: ["Blue Bottle", "Sweetgreen", "DoorDash", "Chipotle", "Levy Catering", "Aramark"],
    ExpenseCategory.ADVERTISING: ["Meta Ads", "Google Ads", "TikTok Ads", "Spotify Ads", "iHeartMedia", "Reddit Ads"],
    ExpenseCategory.PROFESSIONAL_SERVICES: ["Gunderson Dettmer", "Deloitte", "PwC", "Ernst & Young", "Cooley LLP"],
    ExpenseCategory.OFFICE: ["Staples", "Amazon Business", "WB Mason", "Costco Biz"],
    ExpenseCategory.UTILITIES: ["Con Edison", "Comcast Biz", "AT&T Business"],
    ExpenseCategory.RENT: ["SL Green", "WeWork", "Hudson Pacific"],
    ExpenseCategory.SHIPPING: ["FedEx", "UPS", "Shippo", "USPS Biz"],
    ExpenseCategory.FUEL: ["Shell", "Chevron", "76 Station"],
    ExpenseCategory.OTHER: ["Square Vendor", "Stripe Vendor", "Misc Vendor"],
}

# Merchants used to host fraud bursts — high-risk, card-not-present, foreign cash-out
# points (compromised corporate cards drained into gift cards, crypto and grey-market resale).
FRAUD_MERCHANT_NAMES = ["GiftCardVault", "CryptoTopUp Pro", "QuickPrepaid", "GreyMarket Resale", "EverGadget Outlet"]
...
DOMESTIC_MULE_NAMES = ["Peer2Peer Cash US", "ResaleHub US", "QuickResale US", "PrepaidHub US"]
...
class GenSpec:
    ...
    seed: int = 7
    company_name: str = "SeatGeek"
    industry: str = "Live-Event Ticketing Marketplace"
    n_employees: int = 42
    n_merchants: int = 64
    days: int = 90
    ...
    cash_balance_usd: float = 12_000_000.0
...
class _Builder:
    ...
    def __init__(self, spec: GenSpec):
        ...
        self.cards: list[SeatGeekCard] = []
        self.cash_accounts: list[SeatGeekCashAccount] = []
    ...
    def build_cash_accounts(self) -> None:
        ...
        self.cash_accounts = [
            SeatGeekCashAccount(id="cash_operating", company_id=self.company.id,
                            balance_cents=int(bal * 0.45), apy=0.0, account_type="operating"),
            SeatGeekCashAccount(id="cash_yield", company_id=self.company.id,
                            balance_cents=int(bal * 0.45), apy=s.cash_apy, account_type="yield"),
            SeatGeekCashAccount(id="cash_reserve", company_id=self.company.id,
    ...
    def build_employees_and_cards(self) -> None:
        ...
        for i in range(s.n_employees):
            ...
            emp = Employee(
                ...
                email=f"{name.split()[0].lower()}.{name.split()[1].lower()}@seatgeek.com",
            ...
            self.cards.append(SeatGeekCard(
    ...
    def build_legit_transactions(self) -> None:
        ...
        for emp, card in zip(self.employees, self.cards):
            ...
            travels = emp.department in ("Partnerships", "Executive")
            ...
            for _ in range(n_txn):
                ...
                # Most spend is home-metro on a shared/office or personal IP; Partnerships &
                # Executive legitimately travel to venues, producing geo jumps on a TRUSTED device.
                geo = emp.home_geo
    ...
    def build_cash_transactions(self) -> None:
        """SeatGeek Treasury money movement, generated to look like a real operating account.
        ...
        for d, total in sorted(by_day.items()):
            self.cash_txns.append(CashTransaction(
                ...
                counterparty="SeatGeek Card Settlement", memo="Nightly card spend sweep"))
        ...
        # Marketplace revenue: ticketing settlements from the payment processors and
        # partners that remit SeatGeek's take-rate, each on its own cadence and phase, so
        ...
        for c, (cad, share) in enumerate(zip(cadences, shares)):
            ...
            while d < s.days:
                ...
                self.cash_txns.append(CashTransaction(
                    ...
                    counterparty=f"Ticketing Settlement — Processor {chr(65 + c)}", memo="Marketplace take-rate ACH"))
        ...
        self.cash_txns.append(CashTransaction(
            ...
            counterparty="Enterprise Venue Partner — Annual Prepay", memo="Annual venue partnership prepayment"))
        self.cash_txns.append(CashTransaction(
        ...
        # Monthly SeatGeek Treasury yield accrual on the yield account.
        for mo in range(max(1, s.days // 30)):
            ...
            self.cash_txns.append(CashTransaction(
                ...
                counterparty="SeatGeek Treasury", memo=f"Yield accrual @ {yld.apy:.2%} APY"))
...
def generate_tenant(spec: GenSpec | None = None, *, seed: int | None = None) -> Dataset:
    """Generate one deterministic synthetic SeatGeek tenant.
