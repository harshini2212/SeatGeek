"""Render genuine receipt images from SeatGeek Card transactions.
...
def build_sample_receipts(dataset: Dataset, *, seed: int = 7) -> list[GeneratedReceipt]:
    ...
    Only transactions that are *uniquely* identifiable by (merchant, day) are eligible,
    so the autopilot's content-based match resolves back to exactly one transaction —
    the round-trip stays deterministic regardless of how the tenant's RNG stream falls.
    """
    rng = np.random.default_rng(seed + 909)
    # The matcher identifies a receipt by merchant *name* + day, so count non-fraud
    # transactions per (merchant name, calendar day) and keep only the singletons —
    # different merchant IDs can share a name, so keying on the name is what makes the
    # round-trip unambiguous.
    name_of = dataset.merchant_index()
    seen: dict[tuple[str, object], int] = {}
    for t in dataset.card_transactions:
        if not t.ground_truth.is_fraud:
            key = (name_of[t.merchant_id].name, t.ts.date())
    ...
    pool = [t for t in dataset.card_transactions
            ...
            and t.amount > 25
            and seen.get((name_of[t.merchant_id].name, t.ts.date())) == 1]
