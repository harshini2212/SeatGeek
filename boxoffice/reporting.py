"""Rich console reporting for BoxOffice — leaderboards, alerts, investigations.

Centralizes all human-facing output so the CLI and demo render consistently. Uses a
SeatGeek-flavored accent and degrades gracefully on legacy Windows terminals.
"""
...
ACCENT = "dodger_blue2"   # SeatGeek blue
GOOD = "green3"
WARN = "yellow3"
BAD = "red3"
...
def banner() -> None:
    _console.print(Panel.fit(
        Text.assemble(
            ("BOXOFFICE\n", f"bold {ACCENT}"),
            ("Agentic AI marketplace trust + financial-correctness evaluation for SeatGeek", "white"),
        ),
...
def dataset_summary(summary: dict[str, Any]) -> None:
    t = Table(title="Synthetic SeatGeek tenant", title_style=f"bold {ACCENT}", show_header=False,
              box=None)
...
def orchestrator_decision(decision: dict[str, Any]) -> None:
    ...
    body.append(f"\nfinancial impact ${decision['financial_impact_usd']:,.0f}  |  "
                ...
                style="dim")
    _console.print(Panel(body, title=f"BoxOffice decision — {decision['txn_id']}",
                         border_style=ACCENT, title_align="left"))
