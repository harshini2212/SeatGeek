...
SeatGeek Card, open a network dispute, sweep the whole ring, rotate the device). The
...
class FraudInvestigator:
    ...
    def _actions(band: RiskBand, in_ring: bool, n_members: int) -> list[str]:
        ...
        if band in (RiskBand.CRITICAL, RiskBand.HIGH):
            actions.append("Freeze the SeatGeek Card immediately")
            ...
            if in_ring:
                ...
                actions.append("Escalate to the SeatGeek fraud-ops on-call")
