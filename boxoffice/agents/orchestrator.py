"""BoxOfficeOrchestrator — the autonomous agent that runs the whole desk.

For any SeatGeek Card transaction it sequences the specialist agents — categorize, audit
...
class BoxOfficeOrchestrator:
    ...
    def handle_transaction(self, txn_id: str) -> OrchestratorDecision:
        ...
        trace: list[str] = [f"BoxOffice agent engaged on {txn_id} via backend "
                            f"'{self.backend.name}'."]
        total_usage = Usage()
        total_latency = 0.0
